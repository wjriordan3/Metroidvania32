...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") && switch_type == SwitchType.BUTTON:
...
func _on_damage_area_damage_taken(attack_area: Variant) -> void:
	...
	if switch_type == SwitchType.GUN && attack_area.get_collision_layer_value(13):
		_activate()
	elif switch_type == SwitchType.KICK && attack_area.get_collision_layer_value(11):
		_activate()
